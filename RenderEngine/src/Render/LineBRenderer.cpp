...
namespace GLRhi
{
    bool LineBRenderer::initialize(QOpenGLFunctions_3_3_Core* gl)
    {
        ...
        if (!m_gl)
        {
            assert(!"LineBRenderer initialize: OpenGL functions not available");
            return false;
        }
        // 初始化着色器
    ...
    void LineBRenderer::render(const float* cameraMat)
    {
        ...
        for (auto& lineBInfo : m_lineBInfos)
        {
            ...
            {
                // 设置线的参数
                m_program->setUniformValue(m_colorLoc, QVector4D(lineBInfo.color.r(), lineBInfo.color.g(), lineBInfo.color.b(), lineBInfo.color.a()));
    ...
    void LineBRenderer::cleanup()
    {
        ...
        for (auto& lineBInfo : m_lineBInfos)
        {
            if (lineBInfo.vao) m_gl->glDeleteVertexArrays(1, &lineBInfo.vao);
            if (lineBInfo.vbo) m_gl->glDeleteBuffers(1, &lineBInfo.vbo);
            lineBInfo.vao = 0;
            lineBInfo.vbo = 0;
